...
import type { ImpactVideoComment } from "@/lib/impact-interaction";

interface ImpactVideoCommentsSectionProps {
  videoSlug: string;
  initialComments?: ImpactVideoComment[];
}
...
export function ImpactVideoCommentsSection({
  videoSlug,
  initialComments = [],
}: ImpactVideoCommentsSectionProps) {
  const [comments, setComments] = useState<ImpactVideoComment[]>(
    initialComments
  );
  ...
  useEffect(() => {
    if (!mounted) return;
    fetch(`/api/impact/video/${videoSlug}/comments`)
      .then((res) => res.json())
      .then((data) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/impact/video/${videoSlug}/comments`, {
        method: "POST",
  ...
  return (
    <section
      ...
      aria-labelledby="impact-video-comments-heading"
    >
      <h2
        id="impact-video-comments-heading"
      ...
      >
        ...
        <div>
          <Label htmlFor="impact-video-comment-name">Your name</Label>
          <Input
            id="impact-video-comment-name"
            value={authorName}
        ...
        <div>
          <Label htmlFor="impact-video-comment-content">Comment</Label>
          <Textarea
            id="impact-video-comment-content"
            value={content}
