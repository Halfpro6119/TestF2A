...
import type { ImpactArticleComment } from "@/lib/impact-interaction";

interface ImpactCommentsSectionProps {
  articleSlug: string;
  initialComments?: ImpactArticleComment[];
}
...
export function ImpactCommentsSection({
  articleSlug,
  initialComments = [],
}: ImpactCommentsSectionProps) {
  const [comments, setComments] = useState<ImpactArticleComment[]>(
    initialComments
  );
  ...
  useEffect(() => {
    if (!mounted) return;
    fetch(`/api/impact/article/${articleSlug}/comments`)
      .then((res) => res.json())
      .then((data) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/impact/article/${articleSlug}/comments`, {
        method: "POST",
  ...
  return (
    <section
      ...
      aria-labelledby="impact-comments-heading"
    >
      <h2
        id="impact-comments-heading"
      ...
      >
        ...
        <div>
          <Label htmlFor="impact-comment-name">Your name</Label>
          <Input
            id="impact-comment-name"
            value={authorName}
        ...
        <div>
          <Label htmlFor="impact-comment-content">Comment</Label>
          <Textarea
            id="impact-comment-content"
            value={content}
